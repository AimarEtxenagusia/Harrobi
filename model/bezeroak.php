...
// Bezeroak klasea: bezero bateko datuak eta DB-rekin CRUD operazioak
class Bezeroak
{
    public $id;
    public $izena;
    ...
    // Konstruktorea, klasea sortzerakoan datuak sartzeko
    ...
    // Getters eta setters, behar izanez gero erabiltzeko
    ...
    public function getInstalazioa() { return $this->instalazioa; }
    public function setInstalazioa($instalazioa) { $this->instalazioa = $instalazioa; }

    // DB-tik bezero guztiak lortu
    ...
    public static function gehituBezeroa($conn, $izena, $abizena, $email, $pasahitza, $nan, $instalazioa)
    {
        ...
        if ($stmt->execute()) {
            header("Location: ../bezeroa/bezeroa.php"); // buelta
            exit();
        } else {
    ...
    public static function aldatuBezeroa($conn, $izena, $abizena, $email, $pasahitza, $nan, $instalazioa, $id)
    {
        ...
        if ($stmt->execute()) {
            header("Location: ../bezeroa/bezeroa.php"); // editatu ondoren itzuli
            exit();
        } else {
            echo '<div class="alert alert-danger mt-3">Errorea: ' . $stmt->error . '</div>';
        }
    ...
    // Bezero bat bilatu IDaren arabera
